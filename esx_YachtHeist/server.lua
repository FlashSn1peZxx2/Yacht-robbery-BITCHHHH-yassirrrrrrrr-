...
AddEventHandler('esx_YachtHeist:safeRobbed', function(id, state)
	...
	TriggerClientEvent("esx:showNotification",source,"You received: ~r~$"..cashReward.."~s~ money and ~b~"..itemReward.."x~s~ ~y~Gold Necklace~s~")
end)
...
ESX.RegisterServerCallback("esx_YachtHeist:getDrillItem",function(source,cb)
	...
	if xPlayer.getInventoryItem(Config.DrillItem).count >= 1 then
		...
		cb(false)
		TriggerClientEvent("esx:showNotification",source,"You need a ~y~Blowtorch~s~ to drill the safe!")
	end
end)
